        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              ...
              style={[
                styles.cell,
                { backgroundColor: theme.bgCard, borderColor: item.id === selected ? item.color : theme.border },
              ]}
            >
              ...
              {item.id === selected && (
                <Ionicons name="checkmark-circle" size={16} color={item.color} style={styles.check} />
              )}
            </Pressable>
          )}
          ListFooterComponent={
            <Pressable onPress={onAddNew} style={[styles.addNew, { borderColor: theme.border }]}>
              ...
              <Text style={[styles.addNewLabel, { color: theme.primary }]}>Add new</Text>
            </Pressable>
          }
        />
      </View>
...
const styles = StyleSheet.create({
  ...
  cell: {
    flex: 1, margin: 4, borderRadius: 16, borderWidth: 2,
