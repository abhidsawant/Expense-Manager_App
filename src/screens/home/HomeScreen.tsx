...
export default function HomeScreen({ navigation }: any) {
    ...
    return (
      ...
      >
        ...
        <View style={styles.rowText}>
          ...
          {item.note ? <Text style={[styles.note, { color: theme.textMuted }]} numberOfLines={1}>{item.note}</Text> : null}
        </View>
        <View style={[styles.amountBadge, { backgroundColor: theme.primaryLight }]}>
          <Text style={[styles.amount, { color: theme.primary, fontSize: rs(14, 12) }]}>
  ...
  const renderSectionHeader = useCallback(({ section: { title, data } }: any) => {
    ...
    return (
      <View style={[styles.sectionHeader, { backgroundColor: theme.bg, paddingHorizontal: hPad - 16 }]}>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.bg }]} edges={['top']}>
      <View style={[styles.header, { paddingHorizontal: hPad }]}>
        <View>
          <Text style={[styles.greeting, { color: theme.textMuted }]}>{t('home.greeting', { name: settings.username })}</Text>
          <Text style={[styles.headerTitle, { color: theme.text, fontSize: rs(26, 22, 32) }]}>{t('home.title')}</Text>
        </View>
        <Pressable onPress={() => navigation.navigate('About')} style={[styles.headerBtn, { backgroundColor: theme.bgCard, borderColor: theme.border }]}>
...
const styles = StyleSheet.create({
  ...
  listEmpty: { flex: 1 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, marginTop: 6 },
  ...
  row: { flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 18, borderWidth: 1, padding: 14, marginBottom: 8, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  fab: { position: 'absolute', right: 24, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.35, shadowRadius: 16, elevation: 12 },
